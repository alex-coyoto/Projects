                ...
                task.on('state_changed', 
                    ...
                    function complete(){
                        ...
                        auth.onAuthStateChanged(firebaseUser => {
                            if(firebaseUser){
                                var date = new Date();
                                updateSqlDB(firebaseUser.uid, currentId, `${document.querySelector('#name').value} - ${document.querySelector('#author').value}`,
                                    `${date.toDateString()} ${date.toTimeString().split(' ')[0]}`);
                            }       
                        }); 
                        function updateSqlDB(userId, trackId, trackName, date){
                            var request = new XMLHttpRequest();
                            var body = `?userId=${userId}&trackId=${trackId}&trackName=${trackName}&date=${date}`;
                            request.open('GET', "http://localhost:5500/add"+body);
