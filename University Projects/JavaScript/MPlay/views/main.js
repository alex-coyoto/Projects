...
let Main = {
    ...
    renderSide: (side) =>{
        ...
        db.ref('tracks/').on('value', function(snapshot) {
            ...
            for(var i=snapshot.val().length-1;i>=0;i--)
            {
                ...
                //<img class="albumImage" id="albumId${currId}" src="${snapshot.val()[i].image}">
    ...
    likeButtonClicked: (target, buttonId) => {
        if(auth.currentUser)
        {
            ...
            target.src == `${imgAdress}like.png` ? target.src =
             `${imgAdress}like_red.png` : target.src = `${imgAdress}like.png`;
